...
ggplot(data = reglas_df, aes(x = reorder(items, support), y = support)) +
  geom_bar(stat = "identity") +
  labs(x = "", y = "Proporcion (>= 0.025)", title = "Conbinacion de Sintomas COVID-19", subtitle = subtitulo) +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
  coord_flip()
```
...
ggplot(data = reglas_decesos, aes(x = reorder(items, pct_muertes), y = pct_muertes)) +
  geom_bar(stat = "identity") +
  labs(x = "", y = "% descesos", title = "Pct. Decesos por Conbinacion de Sintomas COVID-19") +
