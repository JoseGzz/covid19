...
```{r}
# Registros de síntomas a variables dummy.
datos_snulos <- crudos %>%
  drop_na(symptom) %>%
  filter(symptom != "")
...
niveles <- split %>%
  unlist() %>% # lista plana.
  unique() # valores únicos de síntomas.
...
ggplot(data = sumas_df, aes(x = reorder(sintomas, proporcion), y = proporcion)) +
  ...
  labs(x = "", y = "Proporción", title = "Frecuencia de Síntomas COVID-19", subtitle = sub_titulo) +
  coord_flip() +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5))
```

```{r}
# Selección de variables y formato para Apriori.
top_n_sintomas <- 20
...
# Sólo queremos combinaciones donde los síntomas estén presentes.
valores <- c()
...
```{r}
reglas_df <- as.data.frame(inspect(rules))
reglas_df$items %<>%
  lapply(
...
ggplot(data = reglas_df, aes(x = reorder(items, support), y = support)) +
  geom_bar(stat = "identity") +
  labs(x = "", y = "Proporción (>= 0.025)", title = "Conbinación de Síntomas COVID-19", subtitle = subtitulo) +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5)) +
  coord_flip()
```
```{r}
datos_sintomas <- sintomas_id %>%
  inner_join(datos_snulos %>% select(id, age, death), by = "id")
datos_sintomas
```
```{r}
...
ggplot(data = reglas_decesos, aes(x = reorder(items, pct_muertes), y = pct_muertes)) +
  geom_bar(stat = "identity") +
  labs(x = "", y = "% descesos", title = "Pct. Decesos por Conbinación de Síntomas COVID-19") +
  theme(plot.title = element_text(hjust = 0.5), plot.subtitle = element_text(hjust = 0.5), title = element_text(size = 10)) +
  coord_flip()
```
```{r}

# ("1/9", "1/11", "1/17", "1/16", "1/27",  "1/96")

datos_sintomas %>%
  filter(fever == 1, cough == 1) %>%
  group_by(muerte) %>%
  summarise(edad_prom = median(age))

edades <- c(40, 44, 44, 65, 44)
```
